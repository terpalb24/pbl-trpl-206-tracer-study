                    ...
                    <form id="questionnaireForm" method="POST" action="{{ route('company.questionnaire.submit', $periode->id_periode) }}">
                                    ...
                                    <div class="border-t border-gray-300 pt-4">
                                            ...
                                            <div class="bg-white border border-gray-300 rounded-lg p-4">
                                                <div class="space-y-3">
                                                    @foreach($question->options as $option)
                                                        ...
                                                        @if($option->is_other_option)
                                                            <div class="ml-6 mt-2 p-3 bg-blue-50 border border-blue-200 rounded-md {{ isset($prevMultipleAnswers[$question->id_question]) && in_array($option->id_questions_options, $prevMultipleAnswers[$question->id_question]) ? '' : 'hidden' }}">
                                                                <div class="flex items-center">
                                                                    <i class="fas fa-edit text-blue-600 mr-2"></i>
                                                                    @if($option->other_before_text)
                                                                        <span class="text-gray-600 mr-2">{{ $option->other_before_text }}</span>
                                        ...
                                        @elseif($question->type == 'scale')
                                            ...
                                            <div class="bg-white border border-gray-300 rounded-lg p-4">
                                                ...
                                                <div class="flex items-center justify-between mb-4">
                                                    <span class="text-sm text-gray-600 font-medium">{{ $question->scale_min_label ?? 'Sangat Kurang' }}</span>
                                                    <span class="text-sm text-gray-600 font-medium">{{ $question->scale_max_label ?? 'Sangat Baik' }}</span>
                                                </div>
                        ...
                        <div class="mt-8 pt-6 border-t border-gray-200">
                            <div class="flex justify-between items-center">
                                <div>
                                    @if($prevCategory)
                                        <a href="{{ route('company.questionnaire.fill', [$periode->id_periode, $prevCategory->id_category]) }}" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.option-radio').forEach(radio => {
        radio.addEventListener('change', function() {
            ...
            if (isOther) {
                const otherField = document.getElementById(`other_${this.value}`);
                if (otherField) {
                    otherField.closest('div').classList.remove('hidden');
                }
            }
    ...
    document.querySelectorAll('.multiple-checkbox').forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            ...
            if (isOther) {
                const otherField = document.getElementById(`multiple_other_${this.value}`);
                if (otherField) {
                    if (this.checked) {
                        otherField.closest('div').classList.remove('hidden');
                    } else {
                        otherField.closest('div').classList.add('hidden');
                        otherField.value = '';
                    }
                }
            }
        ...
        document.querySelectorAll('.conditional-question').forEach(question => {
            ...
            if (dependsOn == parentId) {
                if (dependsValue == selectedValue) {
                    ...
                    console.log(`Showing conditional question ${question.id}`);
                } else {
                    question.style.display = 'none';
                    console.log(`Hiding conditional question ${question.id}`);
                }
            }
        });
    }
    ...
    document.querySelectorAll('.option-radio:checked').forEach(radio => {
        ...
        if (parseInt(radio.getAttribute('data-is-other')) === 1) {
            const otherField = document.getElementById(`other_${selectedValue}`);
            if (otherField) {
                otherField.closest('div').classList.remove('hidden');
            }
        }
    ...
    document.querySelectorAll('.multiple-checkbox:checked').forEach(checkbox => {
        ...
        if (isOther === 1) {
            const otherField = document.getElementById(`multiple_other_${checkbox.value}`);
            if (otherField) {
                otherField.closest('div').classList.remove('hidden');
            }
        }
    ...
    function initializeLocationQuestions() {
        ...
        document.querySelectorAll('.province-select').forEach(select => {
            select.addEventListener('change', function() {
                ...
                console.log('Province changed:', { questionId, provinceId });
                
                if (provinceId) {
                    loadCities(questionId, provinceId);
                } else {
        ...
        document.querySelectorAll('.city-select').forEach(select => {
            select.addEventListener('change', function() {
                const questionId = this.getAttribute('data-question-id');
                console.log('City changed for question:', questionId);
                updateLocationDisplay(questionId);
            });
        });
    ...
    function loadProvinces(selectElement) {
        ...
        fetch('/company/questionnaire/provinces')
            ...
            .catch(error => {
                ...
                selectElement.innerHTML = '<option value="">Error loading provinces</option>';
            });
    }
    ...
    function loadCities(questionId, provinceId) {
        ...
        fetch(`/company/questionnaire/cities/${provinceId}`)
            .then(response => response.json())
            .then(data => {
    ...
    function updateLocationDisplay(questionId) {
        ...
        if (provinceName && cityName && provinceName !== '-- Pilih Provinsi --' && cityName !== '-- Pilih Kota/Kabupaten --') {
            ...
            if (combinedInput) combinedInput.value = JSON.stringify(locationData);
    ...
    function initializePreSelectedLocations() {
        ...
        @if(isset($prevLocationAnswers) && is_array($prevLocationAnswers))
            @foreach($prevLocationAnswers as $questionId => $locationJson)
                try {
                    const locationData = JSON.parse('{{ addslashes($locationJson) }}');
                    console.log('Pre-selected location for question {{ $questionId }}:', locationData);
                    
                    if (locationData.province_id && locationData.city_id) {
                        const provinceSelect = document.getElementById('province-{{ $questionId }}');
                        const citySelect = document.getElementById('city-{{ $questionId }}');
                        ...
                        if (provinceSelect) {
                            ...
                            fetch(`/company/questionnaire/cities/${locationData.province_id}`)
                                .then(response => response.json())
                                .then(data => {
                                    if (data.data && Array.isArray(data.data)) {
                                        citySelect.innerHTML = '<option value="">-- Pilih Kota/Kabupaten --</option>';
                                        ...
                                        // Select the pre-selected city
                                        citySelect.value = locationData.city_id;
                                        updateLocationDisplay('{{ $questionId }}');
                                    }
                                })
                                .catch(error => console.error('Error loading cities for pre-selected location:', error));
                        }
                    }
                } catch (e) {
                    console.error('Error parsing pre-selected location data for question {{ $questionId }}:', e);
                }
            @endforeach
        @endif
    }
