    ...
    <main class="flex-grow overflow-y-auto" id="main-content">
        ...
        <div class="bg-white shadow-sm p-4 flex justify-between items-center">
            <div class="flex items-center">
                ...
                <h1 class="text-2xl font-bold text-blue-800">Tambah Kategori</h1>
            </div>
        ...
        <div class="p-6">
            <div class="bg-white rounded-xl shadow-md p-6">
                <form action="{{ route('admin.questionnaire.category.store', $periode->id_periode) }}" method="POST">
                    @csrf
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="for_type" id="for_type" required
                            class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                            <option value="both" {{ old('for_type') == 'both' ? 'selected' : '' }}>Alumni & Perusahaan</option>
                            <option value="alumni" {{ old('for_type') == 'alumni' ? 'selected' : '' }}>Alumni Saja</option>
                            <option value="company" {{ old('for_type') == 'company' ? 'selected' : '' }}>Perusahaan Saja</option>
                        </select>
                        @error('for_type')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
