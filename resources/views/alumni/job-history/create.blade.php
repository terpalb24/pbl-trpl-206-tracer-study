            ...
            <div class="bg-white rounded-lg shadow-lg p-6 max-w-8xl mx-auto">
                ...
                <form action="{{ route('alumni.job-history.store') }}" method="POST">
                    ...
                    <div class="space-y-6">
                        ...
                        <!-- Checkbox -->
                        <div>
                            <input type="checkbox" id="is_current" name="is_current" {{ old('is_current', isset($jobHistory) ? !$jobHistory->end_date : false) ? 'checked' : '' }}>
                                ...
                                <div class="w-1/2">
                                    ...
                                    <select name="start_month" id="start_month" class="w-full border rounded px-3 py-2" required>
                                        ...
                                        @foreach(['01'=>'Januari','02'=>'Februari','03'=>'Maret','04'=>'April','05'=>'Mei','06'=>'Juni','07'=>'Juli','08'=>'Agustus','09'=>'September','10'=>'Oktober','11'=>'November','12'=>'Desember'] as $num => $bulan)
                                            <option value="{{ $num }}" {{ old('start_month', isset($jobHistory) ? \Carbon\Carbon::parse($jobHistory->start_date)->format('m') : '') == $num ? 'selected' : '' }}>{{ $bulan }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="w-1/2">
                                    ...
                                    <select name="start_year" id="start_year" class="w-full border rounded px-3 py-2" required>
                                        ...
                                        @for($y = date('Y'); $y >= 1990; $y--)
                                            <option value="{{ $y }}" {{ old('start_year', isset($jobHistory) ? \Carbon\Carbon::parse($jobHistory->start_date)->format('Y') : '') == $y ? 'selected' : '' }}>{{ $y }}</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="flex gap-4">
                                <div class="w-1/2">
                                    ...
                                    <select name="end_month" id="end_month" class="w-full border rounded px-3 py-2">
                                        <option value="">Pilih Bulan</option>
                                        @foreach(['01'=>'Januari','02'=>'Februari','03'=>'Maret','04'=>'April','05'=>'Mei','06'=>'Juni','07'=>'Juli','08'=>'Agustus','09'=>'September','10'=>'Oktober','11'=>'November','12'=>'Desember'] as $num => $bulan)
                                            <option value="{{ $num }}" {{ old('end_month', isset($jobHistory) && $jobHistory->end_date ? \Carbon\Carbon::parse($jobHistory->end_date)->format('m') : '') == $num ? 'selected' : '' }}>{{ $bulan }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="w-1/2">
                                    ...
                                    <select name="end_year" id="end_year" class="w-full border rounded px-3 py-2">
                                        <option value="">Pilih Tahun</option>
                                        @for($y = date('Y'); $y >= 1990; $y--)
                                            <option value="{{ $y }}" {{ old('end_year', isset($jobHistory) && $jobHistory->end_date ? \Carbon\Carbon::parse($jobHistory->end_date)->format('Y') : '') == $y ? 'selected' : '' }}>{{ $y }}</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                        ...
                        <div>
                            ...
                            <select name="salary" id="salary"
                                ...
                                <option value="0-3000000">0 - 3.000.000</option>
                                <option value="3000000-4500000">>3.000.000 - 4.500.000</option>
                                <option value="4500000-5000000">>4.500.000 - 5.000.000</option>
                                <option value="5000000-5500000">>5.000.000 - 5.500.000</option>
                                <option value="6000000-6500000">>6.000.000 - 6.500.000</option>
                                ...
                                <option value="1200000-15000000">>12.000.000 - 15.000.000</option>
                                <option value="15000001-20000000">>15.000.000 - 20.000.000</option>
                                <option value="20000001">>20.000.000</option>
                            </select>
                        </div>
                    </div>
